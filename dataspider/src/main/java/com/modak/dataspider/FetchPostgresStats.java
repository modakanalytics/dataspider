...
/**
 * This class will connect to postgresql sources and fetch the metadata from postgresql source and inserts the same into staging tables.
 * @author modakanalytics
 * @version 1.0
 */
public class FetchPostgresStats extends MetadataFetcher {
    ...
    private static final org.apache.logging.log4j.Logger LOGGER = LogManager.getLogger("FetchPostgresStats");

    static boolean isDriverLoaded = false;

    @Override
    /**
     * this method will connect to postgres database with provided configurations
     */
    public void run() {
        ...
        try {
            ...
            STGroup SQLgroup = new STGroupFile(runAttributes.get("resources_path") + File.separator + "templates"
                + File.separator + "postgres_stats.stg", '$', '$');
