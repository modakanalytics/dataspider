...
/**
 * This class will connect to oracle sources and fetch the metadata from oracle source and inserts the same into staging tables.
 * @author modakanalytics
 * @version 1.0
 */

public class FetchOracleStats extends MetadataFetcher {
    ...
    private static final org.apache.logging.log4j.Logger LOGGER = LogManager.getLogger("FetchOracleStats");

    static boolean isDriverLoaded = false;

    /**
     * this method will connect to oracle database with provided configurations
     */
    @Override
    public void run() {
        ...
        try {
            try {
                ...
                if (connectionObj_source == null) {
                    //if connection objects is null then insert into error logs
            ...
            STGroup SQLgroup = new STGroupFile(runAttributes.get("resources_path") + File.separator + "templates"
                + File.separator + "oracle_stats.stg", '$', '$');
            ...
            FileUtils.writeStringToFile(new File("finish_logger_file.txt"),
    ...
    @Override
    public void
    init(ConnectAttributes connectAttributes, HashMap<String, String> koshAttributes,
        ...
        try {
            ...
            ex.printStackTrace();
            LOGGER.error("Error while calling init() method of FetchOracleStats\n" + ex);
            LoggerUtils.insertLog(this.getClass().getSimpleName(),
                "Error while calling init() method of FetchOracleStats\n" + ex, true);
        }
    }
    ...
    public List<Object[]> customSQLSelectExecutor(String runAttributeName, STGroup SQLgroup, String query_source,
        ...
        if (runAttributes.containsKey(runAttributeName) && runAttributes.get(runAttributeName).equals("Y")) {
            ...
            try {
                ...
                File error_logger_file = new File(query_source + "_SQLerrors.txt");
                try {
                    FileUtils.writeStringToFile(error_logger_file, ex.fillInStackTrace().toString(),
                        Charset.defaultCharset(), true);
                }
                catch (IOException e1) {
                    e1.printStackTrace();
        ...
        if (runAttributeName.equalsIgnoreCase("table_metadata")) {
            List<Object[]> list_of_dba_all_tables = new ArrayList<>();

            for (Object[] objects : list_of_Objects_array) {
                int size = objects.length;
                String s = (String) objects[size - 1];
                if (s != null) {
                    objects[size - 1] = s.replace("\\x00", "").replace('\u0000', ' ');
                }
                list_of_dba_all_tables.add(objects);
            }
    ...
    public void customSQLInsertExecutor(String runAttributeName, List<Object[]> list_of_Objects_array,
        ...
        if (list_of_Objects_array != null) {
            ...
            try {
                ...
                // append errors to an error file
                File error_logger_file = new File(runAttributeName + "_InsertErrors.txt");
                try {
                    FileUtils.writeStringToFile(error_logger_file, ex.fillInStackTrace().toString(),
                        Charset.defaultCharset(), true);
                }
